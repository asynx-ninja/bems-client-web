...
const TopHeader = ({ fetch, userData }) => {
  ...
  useEffect(() => {
    ...
    const fetchAccountVerification = () => {
      const isVerified = userData;
      setAccountVerified(isVerified);
    };
  ...
  return (
    <>
      ...
      {/* POP UP */}
      {popup && !accountVerified && (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-screen bg-black bg-opacity-[25%] w-full z-[90]">
          <div
            ...
            <div className="flex flex-col mx-auto items-center mt-[70px] p-4 space-x-3 relative m-auto">
              <div role="status" className="inline">
                <h1 className="text-center font-medium">
                  Your Account is not verified yet, please complete your
                  account information.
                </h1>
              </div>
              <Link
              ...
              >
                <h1 className="text-bold my-auto font-bold text-sm text-white">
                  GO TO ACCOUNT
                </h1>
              </Link>
            </div>
