...
const EventsInfo = () => {
  ...
  const handlePageChange = ({ selected }) => {
    ...
    const end = start + 10;
    setFilterType(announcements.slice(start, end));
  };
    ...
    <div className="bg-gray-100">
      ...
      <div className="flex flex-col w-[90%] mt-[20px] mx-auto">
        ...
        <div className="font-bold px-[20px] mx-auto mt-[30px] flex justify-between">
          <div>
            <h1 className="text-[38px] text-custom-green-header">
              EVENTS LIST
            </h1>
          </div>
        </div>
        ...
        <div className="mt-5 w-full flex sm:flex-col sm:gap-5 md:flex-row justify-between items-center">
          ...
          <div className="hs-dropdown relative inline-flex sm:[--placement:bottom] md:[--placement:bottom-left]">
            ...
            >
              SORT BY{" "}
              {sortByOpen === "brgy" ? "BARANGAY" : sortByOpen.toUpperCase()}
              <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
        ...
        <div
          className={
            filterType.length === 0
              ...
              : "grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-8 p-4 w-full justify-center items-center"
          }
        >
          {filterType.map((item, i) => (
            <Link
              key={i}
              to={`/events/?id=${id}&brgy=${brgy}&event_id=${item.event_id}&from=${item.brgy}`}
            >
              <div
